...
public:
    ...
    Multiset(const Multiset& ms2); // Create a new multiset from an existing one.
    
    Multiset &operator=(const Multiset &ms2);
    // Make this multiset contain the same values as ms2 does.
    // Return a reference.
    
    ~Multiset(); // Delete all dynamically allocated memory;
...
// If an item appears n1 times in ms1 and n2 times in ms2, it must appear exactly n1-n2 times (at least 1 or does not appear) in result.
// Allow same parameter.

void combine(const Multiset& ms1, const Multiset& ms2, Multiset& result);
// If an item appears n1 times in ms1 and n2 times in ms2, it must appear exactly n1+n2 times in result.
// Allow same parameter.
