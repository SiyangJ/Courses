...
void WeightedJacobiSweep(const double* rhs,const double* input,double* output, const int& N, const double& w)
{
    ...
    for(i=1;i<N;i++)
    {
        ...
        for(j=1;j<N;j++)
        {
            output[curp] = (1-w)*input[curp]+wd4*(
                            input[curp+1] +
                            input[curp-1] +
                            input[curp+dim] +
                            input[curp-dim] +
...
void WeightedJacobiSweep(const DTDoubleArray &rhs,
                         ...
                         DTMutableDoubleArray &output,double w)
{
    int dim = (int)rhs.m();
    int N = dim-1;
    int i=0,j=0;
    ...
    for(i=1;i<N;i++) {
        ...
        output(0,i) = 0;
        for(j=1;j<N;j++) {
            output(j,i) = (1-w)*input(j,i) + wd4*(input(j-1,i) +
                                                  input(j+1,i) +
                                                  input(j,i+1) +
                                                  input(j,i-1) + rhs(j,i));
        }
...
DTMutableDoubleArray Refine(const DTDoubleArray &input)
{
    ...
    DTMutableDoubleArray output(Nout,Nout);
    output = 0;
    ...
    for (j=;j<;j++) {
        for (i=0;i<;i++) {
            ...
            output(2*i+1,2*j) = (input(i,j)+input(i+1,j))*0.5;
    ...
    for(i=1;i<N;i++)
    {
        ...
        for(j=1;j<N;j++)
        {
            if (j%2==0)
            {
                ...
                {
                    output[curp] =(input[(i+1)/2*coarsedim+j/2]+
                                   input[(i-1)/2*coarsedim+j/2])/2;
                }
            }
            else if (i%2==0)
            {
                output[curp] =(input[i/2*coarsedim+(j+1)/2]+
                               input[i/2*coarsedim+(j-1)/2])/2;
            }
            else
            {
                output[curp] =(input[(i+1)/2*coarsedim+(j+1)/2]+
                               input[(i-1)/2*coarsedim+(j+1)/2]+
                               input[(i+1)/2*coarsedim+(j-1)/2]+
...
void MGSolver(double* rhs, double* input, double* aux, int N, double h,
              const int& Nb, const int& Na, const double& w)
{
    ...
    MGSolver(aux, aux+nextwholearraysize, aux+nextwholearraysize*2, h/2, N/2, Nb,Na,w);
