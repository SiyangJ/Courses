...
void Refine(double* input, const int& N)
{
    ...
    for(i=0;i<Nd2;i++)
    {
        for(j=0;j<Nd2;j++)
        {
            ...
            output[curp2] = xcoarse[curp];
            
            output[curp2+1] = (xcoarse[curp]+
                               xcoarse[curp+1])/2;
            
            output[curp2+dim] = (xcoarse[curp]+
                                 xcoarse[curp+coarsedim])/2;
            
            output[curp2+1+dim] = (
                                   xcoarse[curp]+
                                   xcoarse[curp+1]+
                                   xcoarse[curp+coarsedim]+
                                   xcoarse[curp+1+coarsedim])/4;
    ...
    // i=Nd2;
    fill(output+curp2,output+curp2+N,0.0);
}
...
{
    int wholearraysize = (N+1)*(N+1);
    int nextarraysize = (N/2+1)*(N/2+1);
    
    vector<double> xvec(wholearraysize,0);
    double* x = xvec.data();
    double* r = new double[wholearraysize*3+nextarraysize];
