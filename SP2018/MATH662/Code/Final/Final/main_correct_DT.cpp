...
void MGSolver(const DTDoubleArray& rhs, DTMutableDoubleArray& x,
                              const int& Nb, const int& Na, const double& w)
{
    ////////////////////////////////
    // x might not be a N+1 by N+1 matrix
    // For now, we only use its 0:N,0:N part
    ...
    if (N<=4)
    {
        DTMutableDoubleArray temp = ExactSolver(rhs);
        for(int i=1;i<N;i++){
            for(int j=1;j<N;j++){
                x(i,j) = temp(i,j);
            }
