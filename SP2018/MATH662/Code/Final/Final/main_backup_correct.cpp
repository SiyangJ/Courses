...
void Refine(double* input, const int& N)
{
    ...
    int coarsedim = Nd2+1;
    int coarsesize = coarsedim*coarsedim;
    double* xcoarse = input+coarsesize;
    ...
    for(i=1;i<N;i++)
    {
        ...
        for(j=1;j<N;j++)
        {
            if (j%2==0)
            {
                if (i%2==0)
                {
                    output[curp] = xcoarse[i/2*coarsedim+j/2];
                }
                else
                {
                    output[curp] = xcoarse[(i+1)/2*coarsedim+j/2]/2+\
                                   xcoarse[(i-1)/2*coarsedim+j/2]/2;
                }
            }
            else if (i%2==0)
            {
                output[curp] = xcoarse[i/2*coarsedim+(j+1)/2]/2+\
                               xcoarse[i/2*coarsedim+(j-1)/2]/2;
            }
            else
            {
                output[curp] = xcoarse[(i+1)/2*coarsedim+(j+1)/2]/4+\
                               xcoarse[(i-1)/2*coarsedim+(j+1)/2]/4+\
                               xcoarse[(i+1)/2*coarsedim+(j-1)/2]/4+\
                               xcoarse[(i-1)/2*coarsedim+(j-1)/2]/4;
            }
            curp++;
...
void MGSolver(double* rhs, double* input, double* aux, int N,
              const int& Nb, const int& Na, const double& w)
{
    ...
    // The factor of 4
    // From lhs to rhs
    for(int i=0;i<nextwholearraysize;i++)
        aux[i] *= 4;
              ...
              double* errors,const int& ncycle)
{
    ...
    {
        Axminusb(rhs,x,r,N);
        errors[i++]=maxnorm(r,wholearraysize)/h2;
        if (i>ncycle)
            break;
