...
subtitle: \textbf{Due on 01/22/2018 in Class}
...
```{r echo=TRUE}
X <- matrix(c(1,1,2,1,2,0,.5,1,3,-1,4,1,4,-2,2,1),4,4)
X
```
...
```{r echo=TRUE}
students <- data.frame(id = c("001","002","003"), score_A = c(95,97,90), score_B = c(80,75,84))
students[3,2:3]
...
##  28  29  30  31            
dates = matrix(c(" ",7,14,21,28,"NY",8,"MLK",22,29,2,9,16,23,30,3,10,17,24,31,4,11,18,25," ",5,12,19,26," ",6,13,20,27," "), 5, 7)
dates = data.frame(dates)
colnames(dates) = c("Sun","Mon","Tue","Wed","Thu","Fri","Sat")
calendar1 = cbind(0,dates)
calendar2 = data.frame( list( Sun = c(" ",7,14,21,28), Mon = c("NY",8,"MLK",22,29), Tue = c(2,9,16,23,30), Wed = c(3,10,17,24,31), Thu = c(4,11,18,25," "), Fri = c(5,12,19,26," "), Sat = c(6,13,20,27," ")), row.names=NULL)
calendar2
...
subj = rep(c("A","B"), each = 3)
score = c(95, 97, 90, 80, 75, 84)
fscore = cut(score, breaks = c(0,80,90,100), labels = c("C","B","A"), right = FALSE, ordered_result = TRUE)
grade = factor(fscore)
transform(grade)
students4 = data.frame(id,subj,score,grade)
students4
```
...
m = nrow(X)
n = ncol(X)
X_mean = matrix(data=1, nrow=m) %*% cbind(mean(a),mean(b),mean(c),mean(d))
Diff = X - X_mean
X.var = (n-1)^-1*t(Diff) %*% Diff
X.var
```
...
score_A = c(95,97,90)
score_B = c(80,75,84)
score.mean = rep(c(mean(score_A),mean(score_B)), each = 3)
students5 = data.frame(students3, score.mean)
students5
```
    ...
    print("No root.")
      ...
      while ((upper-lower)/2 > tol){
        ...
        if(f(m) == 0){
          root = m
          f.root = f(root)
          estima.prec = (upper-lower)/2
          return(setNames(as.list(c(root,f.root,iter,estim.prec))),c("root","f.root","iter","estim.prec"))
        }else if (f(lower)*f(m) < 0){
          upper = m
        }else 
          lower = m
        iter = iter + 1
        ...
        root = m
        f.root = f(root)
        estim.prec = (upper-lower)/2
        return(setNames(as.list(c(root,f.root,iter,estim.prec)),c("root","f.root","iter","estim.prec")))
                            
}

bisect(f,1,2,tol = 1e-6)
uniroot(f,c(1,2),tol=1e-6)
```
...
D.inv <- matrix(0L,ncol = 4, nrow = 4)
for (i in 1:4){
  D.inv[i,i] <- 1/D[i]
}
X.cor <- D.inv %*% X.var %*% D.inv
X.cor <- round(X.cor, digits = 2)
write.table(X.cor, file = "X_cor.txt")
X.cor1 <- read.table("X_cor.txt")
X.cor1
```
...
```{r eval=TRUE}
iriss <- split(iris, sample(rep(1:5, ncol(iris), replace = FALSE)))
iris1 = iriss[1]
iris2 = iriss[2]
iris3 = iriss[3]
iris4 = iriss[4]
...
{
  species <- subset(iris,Species==name)
  sepal_length <- as.vector(species$Sepal.Length)
  
  h <- hist(sepal_length,plot=FALSE)

  plot(h,freq=FALSE,ylim=c(0,1.3),xlim=range(h$breaks),xlab=paste('Sepal_Length\nSpecies = ',name,sep=''),main='Histogram of Sepal_Length')
  
  lines(density(sepal_length),col='blue')
  
  curve(dnorm(x,mean(sepal_length),sd(sepal_length)),add=TRUE,col='red')
  
  legend('topright',c('Kernel Density','Normal Density'),col=c('blue','red'),lty=c(1,1),cex=0.5)
